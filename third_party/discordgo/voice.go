...
type VoiceConnection struct {
	...
	// ssrcUsers maps an audio SSRC to the speaking user's ID (populated from
	// OP5 Speaking events). Needed to pick the right per-sender key when
...
type VoiceSpeakingUpdate struct {
	...
	SSRC   int    `json:"ssrc"`
	// Speaking is a bitmask flag (1=voice, 2=soundshare, 4=priority). Discord
	// sends it as a number, so this must be an int, not a bool.
...
func (v *VoiceConnection) onEvent(isBinary bool, message []byte) {
	...
	case 5:
		...
		// Record SSRC -> user so DAVE receive can pick the sender's key.
		if voiceSpeakingUpdate.SSRC != 0 && voiceSpeakingUpdate.UserID != "" {
			v.ssrcUsers.Store(uint32(voiceSpeakingUpdate.SSRC), voiceSpeakingUpdate.UserID)
		}
...
func (v *VoiceConnection) opusReceiver(udpConn *net.UDPConn, close <-chan struct{}, c chan *Packet) {
	...
	for {
		...
		if plain, err := v.aead.Open(nil, nonce[:], cipherTextPayload, recvbuf[:aadLen]); err == nil {
			...
			// DAVE E2EE: the transport-decrypted payload is still an E2EE frame
			// when DAVE is active. Decrypt it with the sender's per-sender key.
			// Passthrough (unencrypted) frames are returned unchanged.
			v.RLock()
			dave := v.dave
			v.RUnlock()
			if dave != nil {
				uid := ""
				if u, ok := v.ssrcUsers.Load(p.SSRC); ok {
					uid, _ = u.(string)
				}
				dec, derr := dave.DecryptFrame(uid, plain)
				if derr != nil {
					// Encrypted frame we can't decrypt yet (sender unknown or no
					// keys). Drop it rather than feed ciphertext to the decoder.
					continue
				}
				plain = dec
			}
