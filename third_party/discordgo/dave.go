...
type DAVESession struct {
	...
	// recvBase caches each speaking user's per-sender base secret, derived from
	// the shared MLS exporter secret. Used to decrypt their DAVE audio frames on
	// receive. Invalidated whenever the exporter secret changes (new epoch).
	recvBase map[string][]byte
}
...
func (d *DAVESession) HandleWelcome(data []byte) error {
	...
	d.recvBase = nil // exporter secret changed; drop cached per-sender secrets
	return nil
}
